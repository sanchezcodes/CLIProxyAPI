		...
		if result.Success {
			...
			if modelKey != "" {
				if !shouldSkipCredentialCooldown(result.Error) {
					...
					// Anthropic OAuth reports exhausted Extra Usage as a 400
					// invalid_request_error. It is credential quota, not a malformed
					// request, so promote it to 429 and allow auth failover.
...
// isClaudeOutOfExtraUsageMessage reports whether Anthropic rejected one OAuth
// credential because its subscription Extra Usage allowance is unavailable.
...
func isRequestInvalidError(err error) bool {
	...
	// This 400 is specific to the selected OAuth credential. Retrying another
	// credential is valid and the depleted credential must enter quota cooldown.
